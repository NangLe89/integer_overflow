...
/**
 * @ast node
 * @declaredat /home/anonymous/WHILE/trunk/WHILE.ast:16
 * @production LogicExpr : {@link Expr} ::= <span class="component">Left:{@link Expr}</span> <span class="component">Op:{@link Op_b}</span> <span class="component">Right:{@link Expr}</span>;
...
public class LogicExpr extends Expr implements Cloneable {
   ...
   */
  public LogicExpr(Expr p0, Op_b p1, Expr p2) {
    setChild(p0, 0);
    setChild(p1, 1);
    setChild(p2, 2);
  }
   ...
   * @apilevel low-level
   */
  public Op_b getOpNoTransform() {
    return (Op_b) getChildNoTransform(1);
  }
  /**
   ...
   * @declaredat /home/anonymous/WHILE/trunk/TypeChecking.jrag:46
   */
    if (getLeft().type() != Type.Bool) {
      {
   ...
   * @attribute coll
   * @aspect TypeInference
   * @declaredat /home/anonymous/WHILE/trunk/TypeChecking.jrag:53
   */
    if (getRight().type() != Type.Bool) {
      {
        Program ref = (Program) (progAST());
        if (ref != null) {
          ref.Program_errors_contributors().add(this);
